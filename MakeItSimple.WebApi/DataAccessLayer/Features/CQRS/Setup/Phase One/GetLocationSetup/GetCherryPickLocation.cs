...
namespace MakeItSimple.WebApi.DataAccessLayer.Features.CQRS.Setup.Phase_One.GetLocationSetup
{
    public class GetCherryPickLocation
    {

        public class GetCherryPickLocationResult
        {
        ...
        public class GetCherryPickLocationQuery : UserParams, IRequest<PagedList<GetCherryPickLocationResult>>
        {
        ...
        public class Handler : IRequestHandler<GetCherryPickLocationQuery, PagedList<GetCherryPickLocationResult>>
        {
            ...
            public async Task<PagedList<GetCherryPickLocationResult>> Handle(GetCherryPickLocationQuery request, CancellationToken cancellationToken)
            {
                ...
                if (requestConcernsQuery.Any())
                {
                    ...
                    if (!string.IsNullOrEmpty(request.UserType))
                    {
                        if (request.UserType == TicketingConString.Requestor)
                        {
                            ...
                            {
                                return new PagedList<GetCherryPickLocationResult>(new List<GetCherryPickLocationResult>(), 0, request.PageNumber, request.PageSize);
                            }
                        }
                ...
                var results = requestConcernsQuery
                    .Select(g => new GetCherryPickLocationResult
                    {
                        Id = g.RequestConcern.LocationId,
                        LocationCode = g.RequestConcern.OneChargingMIS.location_code,
                        LocationName = g.RequestConcern.OneChargingMIS.location_name,

                    }).Distinct();

                return await PagedList<GetCherryPickLocationResult>.CreateAsync(results, request.PageNumber, request.PageSize);
            }
        }
    }
}
